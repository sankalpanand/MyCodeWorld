...
public class KnapsackProblem {
    ...
    // https://www.youtube.com/watch?v=8LusJS5-AGo&t=1s&pbjreload=10
    ...
    public int bottomUpDP(int val[], int wt[], int W){
        ...
        for(int i=0; i <= val.length; i++){
            for(int j=0; j <= W; j++){
                ...
                // Fill the first row and first column with zero
                if(i == 0 || j == 0){
                    K[i][j] = 0;
                    continue;
                }

                // Formula is here- https://youtu.be/8LusJS5-AGo?t=833
                if(j - wt[i-1] >= 0) {
                    // K[i-1][j] = if current is picked
                    // K[i-1][j-wt[i-1]]
...
//        int r = k.bottomUpDP(val, wt, 30);
        int r = k.bottomUpDP(val2, wt2, 7);
        int r1 = k.topDownRecursive(val2, wt2, 7);
