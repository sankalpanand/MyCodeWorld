...
public class WordLadder {
			...
			for (String str : small) 
			{
				...
				for (int i = 0; i < str.length(); i++) 
				{
					...
					for (char ch = 'a'; ch <= 'z'; ch++) 
					{
						...
						// Set the char a-z at 0th getCellIndex, then 1st getCellIndex and so on
						sb.setCharAt(i, ch);
	...
	public static int ladderLength2(String beginWord, String endWord, Set<String> wordDict) {
		...
		while(!added.contains(endWord))
		{
			...
			{
				// Iterate over entire length of each word. We need an getCellIndex to replace characters.
