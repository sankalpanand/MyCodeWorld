...
public class AssortedMethods {
	...
	/** This method takes an ArrayList of Points. 
	 * It then generates a string by reading x and y coordinates from each point appending brackets and commas. 
	 * @param list
	 * @return String
	 */
	 ...
	 * Step n: n
	 * */
	/** It creates a tree. Left to Right, Top to bottom at each level.
	 * Ordering of elements in input array doesn't matter. Do it on paper to understand it clearly.
	...
	public static TreeNode createTreeFromArray(int[] array) 
	{
		...
		if (array.length > 0) 
		{
			// Make first element as the root
			TreeNode root = new TreeNode(array[0]);
			...
			while (!done) 
			{								
				// Get, but do not remove, the first element in the queue
				TreeNode r = (TreeNode) queue.element();
				...
				if (r.left == null) 
				{
					...
					// Create a node from this data. Set this node in the LST
					r.left = new TreeNode(data);									
					
					// Add the LST root to the queue, we need to go deeper into it
					queue.add(r.left);
				} 
				...
				else if (r.right == null) 
				{
					...
					// Create a node from this data. Set this node in the RST
					r.right = new TreeNode(data);
					
					queue.add(r.right);
				} 
				
				// If both LST and RST are present at the first element in the queue, remove the first element. 
				// We will go 1 level deeper.
				else 
				{
					queue.remove();
				}
				
				// Since i is incremented after the operation, i will be equal to length towards the end of the operation
				if (i == array.length)
					done = true;
			}
	...
	public static TreeNode createMinimalBST(int array[]) 
	{
		...
		return createMinimalBST(array, start, end);
	}
	...
	public static TreeNode createMinimalBST(int arr[], int start, int end)
	{
		...
		// Create the root using the middle element
		TreeNode n = new TreeNode(arr[mid]);

		// Send the remaining elements on its left to the LST
		n.setLeftChild(createMinimalBST(arr, start, mid - 1));

		// Send the remaining elements on its left to the RST
		n.setRightChild(createMinimalBST(arr, mid + 1, end));

		// The LST and RST will be build recursively.
		// When the recusrion finishes, tree is complete, return the root node.
		return n;
	}
}