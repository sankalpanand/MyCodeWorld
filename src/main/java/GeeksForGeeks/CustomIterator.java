...
public class CustomIterator<T> implements Iterable<T> {
    ...
    public CustomIterator(T[] values) {
        this.values = values;
    }

    // This is a private class that implements iteration over the elements of the list.
    // It is not accessed directly by the user, but is used in the iterator() method of the Array class.
    // It implements the hasNext() and next() methods.
    class ArrayIterator implements Iterator<T> {
        int current = 0;  // the current element we are looking at

        // return whether or not there are more elements in the array that have not been iterated over.
        public boolean hasNext() {
            if (current < CustomIterator.this.values.length) {
                return true;
            } else {
                return false;
            }
    ...
    public static void main(String[] args) {
        ...
        CustomIterator<String> array = new CustomIterator<>(strings);
